...
export async function resizeImageHandler(req: NextRequest): Promise<NextResponse> {
    ...
    try {
        const cachedData = await kv.get<{ url: string, lastAccessed: number }>(cacheKey);
        if (cachedData) {
            // Update lastAccessed timestamp without waiting for it to complete
            kv.set(cacheKey, { ...cachedData, lastAccessed: Date.now() });
            return NextResponse.redirect(cachedData.url, 302);
        }
        ...
        const lockAcquired = await kv.set(lockKey, "true", { nx: true, ex: 60 });

        if (lockAcquired) {
            await imageQueue.enqueue({ src, w, q, format, noCdn: false });
        }
